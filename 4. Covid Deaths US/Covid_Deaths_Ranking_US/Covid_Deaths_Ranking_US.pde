...
String title = "Covid Deaths by US States";
...
void setup() {
  ...
  for (int i=2; i<n_cols; i++) {
    ...
    flag_img[i][0].resize(int((max_bar_height - 2*bar_spacing)*5/3), int((max_bar_height - 2*bar_spacing)*3/3));
    flag_img[i][1].resize(adjustedDisplayWidth*18/100, adjustedDisplayHeight*1/15);
  }
} 
  ...
  for(int i=2; i<n_cols; i++) {
    ...
    if(rank_val<n_bars) {
      ...
      if(bar_width > bar_height*5/3) {
        image(flag_img[i][0], left_margin + bar_width - bar_height*5/3, bar_y - bar_height*0/2);
      }
      ...
      textSize(bar_height*28/100);
      textAlign(LEFT);
      text(int(row_values.getFloat(i)), left_margin + bar_width + bar_spacing, bar_y + bar_height/2 + bar_spacing);
      if(rank_val<3) {
        image(flag_img[i][1], adjustedDisplayWidth*2/5 + (adjustedDisplayWidth/5)*rank_val - 10, adjustedDisplayHeight*78/100);
      }
    }
  }
  fill(0, 0, 0);
